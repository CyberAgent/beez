/**
 * @name dispose.js<spec/index>
 * @author Kei Funagayama <funagayama_kei@cyberagent.co.jp>
 * copyright (c) Cyberagent Inc.
 * @overview TestCase: s/beez-mvcr/*#dispose()
 */

define(['beez.mvcr', 'backbone.localStorage'], function(mvcr, LocalStorage) {
    var ManagerBase = mvcr.ManagerBase;
    var Base = mvcr.Base;
    var Model = mvcr.Model;
    var ModelManager = mvcr.ModelManager;
    var Collection = mvcr.Collection;
    var mManager = new ModelManager('midx');
    var View = mvcr.View;
    var ViewManager = mvcr.ViewManager;
    ...
    return function () {
        ...
        describe('managers dispose', function() {
            it('controller', function(done) {
                var TestController = mvcr.Controller.extend(
                    'test.controller.TestController', { dummy: function() { return "dummy"; } } );

                var controller;
                var async = mvcr.manager.controller.async();

                async
                    .create('async', TestController)
                    .then(function onSuccess() {
                        controller = mvcr.manager.controller.get('async');
                        expect(controller.dummy()).equals('dummy');
                        expect(async.hasOwnProperty('manager')).be.ok;
                        done();
                    })
                    ...
                    .end(function(err, ress) {
                        expect(async.hasOwnProperty('manager')).not.be.ok;
                        expect(mvcr.manager.controller.hasOwnProperty('controllers')).be.ok;
                        mvcr.manager.controller.dispose();
                        expect(mvcr.manager.controller.hasOwnProperty('controllers')).not.be.ok;
                    })
                ;
            });
        ...
        describe('Model', function () {
            ...
            it('save', function (done) {
                ...
                .then(function(res, next) {
                    ...
                    expect(res.model.urlRoot).eq('http://0.0.0.0:1109/api');
                    done();
                })
                .error(function(err, next) {
        ...
        describe('View', function () {
            ...
            it('remove', function () {
                testView1 = new TestView1(vManager, {'foo': true});
                testView1.render();
        ...
        describe('ViewManager', function () {
            it('constructor', function () {
                ...
                testViewManager1 = vManager.create('/@', TestView1, {'foo': true});
                testViewManager2 = vManager.create('/@/testVidx1', TestView2, {'foo': true});
            ...
            it('dispose', function () {
                testViewManager3 = vManager.create('/@', TestView3, {'foo': true});
                testViewManager4 = vManager.create('/@/testVidx3', TestView4, {'foo': true});
                testViewManager5 = vManager.create('/@/testVidx3', TestView5, {'foo': true});
                testViewManager6 = vManager.create('/@/testVidx3/testVidx4', TestView6, {'foo': true});
        ...
        describe('ModelManager', function () {
            ...
            it('remove', function () {
                ...
                //expect(mManager.get('/@/test4/test5').isBinded).not.be.ok;
